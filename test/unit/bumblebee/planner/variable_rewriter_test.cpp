...
TEST(ViariableRewriterTest, AddAnonymousRewriting) {
    auto program = getRulesFromFile("rule2");
    EXPECT_EQ(program.size(), 1);
    auto& rule = program[0];
    ...
    EXPECT_EQ(rule.getBody()[0].containsAnonymous(), true);
    EXPECT_EQ(rule.getBody()[1].containsAnonymous(), true);
}


TEST(ViariableRewriterTest, RemoveAtomRewriting) {
